...
public class LoginActivity extends AppCompatActivity {
    ...
    private void loginExecute() {
        ...
        Call<ApiResponse<List<Login>>> loginCall = apiHelper.userLogin(binding.loginid.getText().toString(),binding.password.getText().toString());
        loginCall.enqueue(new Callback<ApiResponse<List<Login>>>() {
            @Override
            public void onResponse(@NonNull Call<ApiResponse<List<Login>>> call,
                                   @NonNull Response<ApiResponse<List<Login>>> response) {
                binding.setLoaderVisibility(false);
                binding.setButtonVisibility(true);
                if (response.isSuccessful()) {
                    ...
                    if (response!=null){
                        if (response.body().getResponse().equalsIgnoreCase("Failure")){
                            ...
                            List<Login> login=response.body().getBody();
                            ...
                            Gson gson=new Gson();
                            String json=gson.toJson(login.get(0),Login.class);
            ...
            @Override
            public void onFailure(@NonNull Call<ApiResponse<List<Login>>> call,
                                  @NonNull Throwable t) {
                if (!call.isCanceled()) {
                    binding.setLoaderVisibility(false);
                    binding.setButtonVisibility(true);
